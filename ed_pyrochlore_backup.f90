...
contains
  ...
  subroutine solve_hk_topological(sigma)
    ...
    if(master)then
       ...
       kpath(2,:)=[0.00, 1.00, 0.00]
       kpath(3,:)=[0.50, 1.00, 0.00]
       kpath(4,:)=[0.75, 0.75, 0.00]
       kpath(5,:)=[0.50, 0.50, 0.50]
       kpath(6,:)=[0.00, 0.00, 0.00]
       kpath=kpath*pi2
       !
       ...
       call solve_nh_model(hk_pyro,Nso,kpath,Nkpath,&
            ...
            points_name=[character(len=20) :: "{\Symbol G}","X","W","K","L","{\Symbol G}"],&
            file="Eig_Htop.ed")
    endif
  ...
  function hk_pyro(kvec,N) result(hk)
    ...
    !
       ...
       sigmapyro(2,2)=-2d0*xi
       sigmapyro(3,3)=-1d0*xi
       sigmapyro(4,4)=-2d0*xi
       sigmapyro(5,5)=sigmapyro(1,1)
       sigmapyro(6,6)=sigmapyro(2,2)
       sigmapyro(7,7)=sigmapyro(3,3)
       sigmapyro(8,8)=sigmapyro(4,4)
    ! !add the Sigmapyro term to get Topologial Hamiltonian if required:
    Hk = Hk + Sigmapyro
    if (usez) then
       Zmats=zero
       do ii=1,Nso
