program ed_ti_slab
  ...
  real(8),dimension(3)                          :: Bfield
  complex(8),dimension(4,4)                     :: emat,soxmat,soymat,sozmat
  real(8)                                       :: e0,mh,lambda,dummymag,dummymag_rescale,radius,alpha
  complex(8),dimension(4,4)                     :: NHmat,GapOpeningMat,magmat,Bmat
  real(8)                                       :: GapOpeningField,NHfield,DUMMY,offset_x,offset_y
  ...
  call parse_input_variable(NHfield,"NHFIELD",finput,default=0.0d0)
  ...
  call parse_input_variable(Bfield,"BFIELD",finput,default=[0d0,0d0,0d0])
  call parse_input_variable(alpha,"ALPHA",finput,default=0d0)
  !
  ...
  !
  soxmat=kron_pauli( pauli_sigma_x, pauli_tau_x)
  soymat=kron_pauli( pauli_sigma_y, pauli_tau_x)
  sozmat=kron_pauli( pauli_sigma_z, pauli_tau_x)
  !
  !
  NHmat = xi*NHFIELD*kron_pauli( pauli_sigma_0, pauli_tau_x)
  Bmat = sin(alpha*pi)*BFIELD(1)*kron_pauli(pauli_sigma_x,pauli_tau_0) +&
         sin(alpha*pi)*BFIELD(2)*kron_pauli(pauli_sigma_y,pauli_tau_0) +&
         sin(alpha*pi)*BFIELD(3)*kron_pauli(pauli_sigma_z,pauli_tau_0) +&
         cos(alpha*pi)*BFIELD(1)*kron_pauli(pauli_sigma_x,pauli_tau_z) +&
         cos(alpha*pi)*BFIELD(2)*kron_pauli(pauli_sigma_y,pauli_tau_z) +&
         cos(alpha*pi)*BFIELD(3)*kron_pauli(pauli_sigma_z,pauli_tau_z)
  !
...
contains
  ...
  function h0_rk_ti(kx,ky,N) result(H)
    ...
    !
    H = (cos(kx) + cos(ky) -Mh )*emat+&
        lambda*(sin(kx)*soxmat + sin(ky)*soymat)  + Bmat + Nhmat
  ...
  function t0_rk_ti(N) result(H)
    ...
    complex(8),dimension(N,N)  :: H
    !
    H = 0.5d0*emat + xi*0.5d0*lambda*sozmat
    !
  ...
  subroutine build_eigenbands(kpath_)
    ...
    !
    kpath(1,:)=[0d0,-1d0,0d0]*pi
    kpath(2,:)=[0d0,0d0,0d0]*pi
    kpath(3,:)=[0d0,1d0,0d0]*pi
  ...
  subroutine bands_2d(num)
    ...
    do ix=1,num
      do iy=1,num
        Hk=ti_edge_model([kx(ix),ky(iy),0d0],Nlat,Nso,pbc=PBC)
