program lancED
  ...
  do while(.not.converged.AND.iloop<nloop)
     ...
     if(.not.betheSC)then
        call dmft_self_consistency(Gmats,Smats,Weiss,one*Hloc,SCtype=cg_scheme)
     else
        if(wGimp)call ed_get_gimp_matsubara(Gmats)
        call dmft_self_consistency(Gmats,Weiss,one*Hloc,SCtype=cg_scheme,wbands=Wband)
     endif
     ...
     if(symOrbs)then
        call ed_chi2_fitgf(comm,Weiss,bath,Hloc,ispin=1,iorb=1)
        call ed_orb_equality_bath(bath,save=.true.)
     else
        call ed_chi2_fitgf(comm,Weiss,bath,Hloc,ispin=1)
     endif
     !
     !MIXING:
     if(.not.mixG0)then
