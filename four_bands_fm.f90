program ed_ti_slab
  ...
  if(Nspin/=2.OR.Norb/=4)stop "Wrong setup from input file: Nspin=2, Norb=4 -> 6Spin-Orbitals"
  Nso  = Nspin*Norb
  ...
  do while(.not.converged.AND.iloop<nloop)
     ...
     do ilat=1,Nineq
       toconverge(ilat,1,:) = Weiss_ineq(ilat,1,1,3,3,:)
       toconverge(ilat,2,:) = Weiss_ineq(ilat,2,2,3,3,:)
       toconverge(ilat,3,:) = Weiss_ineq(ilat,1,1,4,4,:)
       toconverge(ilat,4,:) = Weiss_ineq(ilat,2,2,4,4,:)
     enddo
     !
     ...
     do ilat=1,Nlat
       ...
       dens = dens+(fft_get_density(Gmats(ilat,1,1,4,4,:),beta)+fft_get_density(Gmats(ilat,2,2,4,4,:),beta))/Nlso
     enddo
     if(nread/=0.d0)then
...
contains
  ...
  function h0_rk_ti(kx,kz,N) result(H)
    ...
    H(5,5)=Mh+3*e0
    H(6,6)=Mh+3*e0
    H(3:4,3:4)=Hblock(1:2,1:2)
    H(3:4,7:8)=Hblock(1:2,3:4)
    H(7:8,3:4)=Hblock(3:4,1:2)
    H(7:8,7:8)=Hblock(3:4,3:4)
