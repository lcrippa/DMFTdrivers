...
CU    USES fdjac2,fmin2,lnsrch,qrdcmp,qrupdt,rsolv
      INTEGER i,its,j,k
      REAL*8 den,f,fold,stpmax,sum,temp,test,c(NP),d(NP),fvcold(NP),
     *g(NP),p(NP),qt(NP,NP),r(NP,NP),s(NP),t(NP),w(NP),xold(NP),fmin2
...
39      continue
        ...
        call lnsrch(n,xold,fold,g,p,x,f,stpmax,check,fmin2)
        test=0.
        do 41 i=1,n
          if(abs(fvec(i)).gt.test)test=abs(fvec(i))
41      continue
