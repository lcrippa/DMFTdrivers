...
contains
  ...
  subroutine read_sigma_matsubara(Self)
    ...
    integer                                       :: ilat,ispin,iorb,jorb,ineq
          ...
          do iorb=1,Norb
            if(.not.DIAG_SIGMA)then
              do jorb=1,Norb
               ...
               call sread("impSigma"//trim(suffix),wm,Self_ineq(ilat,ispin,ispin,iorb,jorb,:))
              enddo
            else
              jorb=iorb
              suffix="_l"//reg(txtfy(iorb))//reg(txtfy(jorb))//"_s"//reg(txtfy(ispin))//"_iw_ineq"//reg(txtfy(ilat,Npad=4))//".ed"
  ...
  subroutine build_eigenbands(kpath_)
    ...
    !call get_surface_state([offset,0d0,pi])
    !
  ...
  subroutine get_surface_state(kpoint)
    integer                                   :: ipts,ik,ic,unit,iorb,ilat,io,nrot,u1,u2
    real(8)                                   :: coeff(Nlat*Nso),kpoint(3)
    complex(8),dimension(Nlat*Nso,Nlat*Nso)   :: h,evec
    complex(8),dimension(Nlat*Nso)            :: Eval
    real(8),dimension(Nlat*Nso)               :: RePartEval
    integer                                   :: labels(Nlat*Nso)
    !
    h = ti_edge_model(kpoint,Nlat,Nso,pbc) - XMU*Eye(Nlat*Nso)
    call eig(h,Eval,Evec)
    RePartEval=Real(Eval)
    call sort_array(RePartEval,labels)
    !
    do io=78,83
       coeff=Evec(:,labels(io))*conjg(Evec(:,labels(io)))
       coeff=coeff/norm2(coeff)
       if(abs(coeff(2)+coeff(3))>0.8d0)then
        print*,"Band on site 1 spin up ",io,labels(io)," energy ",Real(Eval(labels(io)))
       endif
       if(abs(coeff(5)+coeff(6))>0.8d0)then
        print*,"Band on site 1 spin dw ",io,labels(io)," energy ",Real(Eval(labels(io)))
       endif
       !if(abs(coeff(116)+coeff(117))>0.8d0)then
       ! print*,"Band on site N spin up ",io,labels(io)," energy ",Real(Eval(labels(io)))
       ! print*,coeff(116),coeff(117)
       !endif
       !if(abs(coeff(119)+coeff(120))>0.8d0)then
       ! print*,"Band on site N spin dw ",io,labels(io)," energy ",Real(Eval(labels(io)))
       ! print*,coeff(119),coeff(120)
       !endif
    enddo
    !
  end subroutine get_surface_state    
