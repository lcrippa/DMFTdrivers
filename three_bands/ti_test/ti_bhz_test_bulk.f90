program ed_bhz
  ...
  real(8)                                     :: e0,GapOpeningField,NHfield_up,NHfield_dw
  ...
  NHmat =zero
  NHmat(1:2,1:2) = xi * NHFIELD_UP * pauli_tau_x
  NHmat(3:4,3:4) = xi * NHFIELD_DW * pauli_tau_x
...
contains
  ...
  subroutine solve_hk_topological(sigma)
    ...
    if(master)then
       ...
       call solve_nh_model(hk_bhz,Nso,kpath,Nkpath,&
