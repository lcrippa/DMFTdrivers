program ed_ti_slab
  ...
  !gamma matrices:
  complex(8),dimension(4,4)                     :: emat,soxmat,soymat,sozmat,bxmat,bymat,bzmat,BIAmat,testmat
  ...
  real(8)                                       :: e0,mh,lambda,wmixing,bx,by,bz,BIA,akrange
  ...
  call parse_input_variable(by,"BY",finput,default=0.0d0)
  ...
  do ilat=1,Nlat
    S0(ilat,:,:,:,:)      = xi*10*imag(Smats(ilat,:,:,:,:,1))+0d0   !THIS IS ONLY A TEST
  enddo
  !do ilat=1,Nlat
...
contains
  ...
  subroutine build_hkr(file)
    ...
    !
    !
    kx = linspace(-0.45d0,0.45d0,num)
    ky = linspace(0.6*pi,1.4*pi,num)
    call start_timer()
    do ix=1,num
      do iy=1,num
  ...
  function ti_edge_model(kpoint,Nlat,N,pbc) result(Hrk)
    ...
    do i=1,Nlat
       Idmin=1+(i-1)*N
       Idmax=      i*N
       Hrk(Idmin:Idmax,Idmin:Idmax)=Hmat !< H(k) + Re(Sigma_iy(:Nso,:Nso;omega=0))
    enddo
    do i=1,Nlat-1
       Idmin=1 + (i-1)*N
       Idmax=        i*N
  ...
  function h0_rk_ti(kx,kz,N) result(H)
    ...
    H = (Mh - e0*(cos(kx) + cos(kz)))*emat+&
         lambda*(sin(kx)*soxmat + sin(kz)*sozmat)+ NHmat 
