program ed_ti_2d_edge
  ...
  !gamma matrices:
  complex(8),allocatable,dimension(:,:)         :: mat0,gamma5,mat1,mat2,mat3,mat4,matMagX,matMagY,matMagZ,symBrk
  complex(8),allocatable,dimension(:,:)         :: mat0_6,gamma5_6,mat1_6,mat2_6,mat3_6,mat4_6,matMagX_6,matMagY_6,matMagZ_6,symBrk_6
  real(8),allocatable,dimension(:)              :: kxgrid
  real(8),dimension(3)                          :: mag
  real(8),dimension(:,:),allocatable            :: kpath
  integer                                       :: Nk,Lz,Nkpath,IOFFS
  real(8)                                       :: e0,mh,lambda,wmixing
  logical                                       :: spinsym,tridiag,lrsym,PBC
  ...
  call parse_input_variable(Lz,"Lz",finput,default=20)
  ...
  Nineq= Lz
  if(lrsym)then
     if(mod(Lz,2)/=0)stop "Wrong setup from input file: Lz%2 > 0 (odd number of sites)"
     Nineq=Lz/2
  ...
  allocate(mat0(4,4),gamma5(4,4),mat1(4,4),mat2(4,4),mat3(4,4),mat4(4,4),matMagX(4,4),matMagY(4,4),matMagZ(4,4),symBrk(4,4))
  allocate(mat0_6(6,6),gamma5_6(6,6),mat1_6(6,6),mat2_6(6,6),mat3_6(6,6),mat4_6(6,6),matMagX_6(6,6),matMagY_6(6,6),matMagZ_6(6,6),symBrk_6(6,6))
  

  !SET 1: original
  
  mat0=kron_pauli( pauli_sigma_0, pauli_tau_0 )
  mat1=kron_pauli( pauli_sigma_0, pauli_tau_x )
  mat2=kron_pauli( pauli_sigma_x, pauli_tau_z )
  mat3=kron_pauli( pauli_sigma_y, pauli_tau_z )
  mat4=kron_pauli( pauli_sigma_0, pauli_tau_y )
  ...
  symBrk=kron_pauli( pauli_sigma_z, pauli_tau_z )

  mat0_6=change4to6(mat0,one)
  gamma5_6=change4to6(gamma5,one)
  mat1_6=change4to6(mat1,one)
  mat2_6=change4to6(mat2,one)
  mat3_6=change4to6(mat3,one)
  mat4_6=change4to6(mat4,one)
  matMagX_6=change4to6(matMagX,one)
  matMagY_6=change4to6(matMagY,one)
  matMagZ_6=change4to6(matMagZ,one)
  symBrk_6=change4to6(symBrk,one)
  ...
  if(bath_type=="replica")then
     ...
     Hsym_basis(:,:,:,:,1)=j2so(mat1_6)  ;lambdasym_vector(:,1)=Mh
     Hsym_basis(:,:,:,:,2)=j2so(matMagX_6) ;lambdasym_vector(:,2)=mag(1)
     Hsym_basis(:,:,:,:,3)=j2so(matMagy_6) ;lambdasym_vector(:,3)=mag(2)
     Hsym_basis(:,:,:,:,4)=j2so(matMagZ_6) ;lambdasym_vector(:,4)=mag(3)
  ...
  do while(.not.converged.AND.iloop<nloop)
     ...
     ! solve the impurities on each inequivalent z-layer
...
contains
  ...
  !+-----------------------------------------------------------------------------+!
  !PURPOSE: build the ti Hamiltonian H(k_x,R_y) on the STRIPE along Y
  !+-----------------------------------------------------------------------------+!
  subroutine build_hkr(file)
    ...
    !SETUP THE H(kx,Ry):
    if(master)then
       write(LOGfile,*)"Build H(kx,y) for ti-stripe:"
       write(*,*)"# of kx-points     :",Nk
       write(*,*)"# of ky-points     :",Nk
       write(*,*)"# of z-layers      :",Nlat
    endif
    !
    if(allocated(Hkr))deallocate(Hkr)
    allocate(Hkr(Nlso,Nlso,Nk**2))
    !
    call TB_set_bk([pi2,0d0,0d0],[0d0,pi2,0d0],[0d0,0d0,pi2])
    call TB_build_model(Hkr,ti_edge_model,Lz,Nso,[Nk,Nk,1],pbc=PBC,wdos=.false.)

    !
    !SETUP THE LOCAL PART Hloc(Ry)
    allocate(tiHloc(Nlso,Nlso))
    tiHloc = sum(Hkr,dim=3)/(Nk**2)
    where(abs(tiHloc)<1d-6)tiHloc=zero
    !
  ...
  subroutine build_eigenbands(kpath_)
    ...
    allocate(colors(Lz,Nso))
    colors = red1
    !colors(1+IOFFS,:) = [red1,blue1,red1,blue1]
    !colors(Lz-IOFFS,:) =[blue1,red1,blue1,red1]
    call solve_nh_model(ti_edge_model,Lz,Nso,kpath,Nkpath,&
         colors_name=colors,&
  ...
  function h0_rk_ti(kx,ky,N) result(H)
    real(8)                    :: kx,ky
    integer                    :: N
    ...
    Hblock=zero
    !
    Hblock = (mh-e0*cos(kx)-e0*cos(ky))*mat1 + gamma5+ lambda*(sin(ky)*mat2 - sin(kx)*mat3) !+ mag(1)*matMagX + mag(2)*matMagy + xi*mag(3)*matMagZ + SB_FIELD*symBrk
    H(1,1)=mh+e0
    H(4,4)=mh+e0
  ...
  function t0_rk_ti(N) result(H)
    ...
    Hblock=zero
    !
    Hblock = -0.5d0*e0*mat1 + xi*0.5d0*lambda*mat4
    H(2:3,2:3)=Hblock(1:2,1:2)
    H(2:3,5:6)=Hblock(1:2,3:4)
    H(5:6,5:6)=Hblock(3:4,3:4)
    H(5:6,2:3)=Hblock(3:4,1:2)
  ...
  function change4to6(mat_in,added_element) result (mat_out)
    complex(8),dimension(4,4)             :: mat_in
    complex(8),dimension(6,6)             :: mat_out
    complex(8)                            :: added_element
    !
    mat_out=zero
    !
    mat_out(1,1)=added_element
    mat_out(4,4)=added_element
    mat_out(2:3,2:3)=mat_in(1:2,1:2)
    mat_out(2:3,5:6)=mat_in(1:2,3:4)
    mat_out(5:6,5:6)=mat_in(3:4,3:4)
    mat_out(5:6,2:3)=mat_in(3:4,1:2)    
    !
  end function change4to6


end program ed_ti_2d_edge
