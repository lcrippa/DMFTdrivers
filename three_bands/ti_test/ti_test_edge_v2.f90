...
program ed_ti_2d_edge
  ...
  !gamma matrices:
  complex(8),allocatable,dimension(:,:)         :: mat0,mat1,mat2,mat3,mat4,matMagX,matMagY,matMagZ,symBrk
  ...
  allocate(mat0(Nso,Nso),mat1(Nso,Nso),mat2(Nso,Nso),mat3(Nso,Nso),mat4(Nso,Nso),matMagX(Nso,Nso),matMagY(Nso,Nso),matMagZ(Nso,Nso),symBrk(Nso,Nso))
  ...
  mat3=kron_pauli( pauli_sigma_z, pauli_tau_z )
  ...
  matMagZ=reshape([one,zero,zero,zero,zero,one,zero,zero,zero,zero,-2d0*one,zero,zero,zero,zero,-2d0*one],[4,4])
  
  symBrk=kron_pauli( pauli_sigma_y, pauli_tau_z )
...
contains
  ...
  subroutine build_hkr(file)
    ...
    !SETUP THE H:
    if(master)then
       write(LOGfile,*)"Build for ti-stripe:"
       write(*,*)"# of k1-points     :",Nk
       write(*,*)"# of k2-points     :",Nk
    ...
    call solve_nh_model(ti_edge_model,Lz,Nso,kpath,Nkpath,&
         colors_name=colors,&
         points_name=[character(len=10) :: "-Y","G","Y","-Z","G","Z"],&
         file="Eigenbands.nint",&
         pbc=PBC)
  ...
  function h0_rk_ti(ky,kz,N) result(H)
    real(8)                    :: ky,kz
    integer                    :: N
    complex(8),dimension(N,N)  :: H
    H = (mh-e0*cos(ky)-e0*cos(kz))*mat1 + lambda*(sin(ky)*mat2 + sin(kz)*mat4) + mag(1)*matMagX + mag(2)*matMagy + mag(3)*matMagZ + SB_FIELD*symBrk
  end function h0_rk_ti

  function t0_rk_ti(N) result(H)
    integer                    :: N
    complex(8),dimension(N,N)  :: H
    H = -0.5d0*e0*mat1 - xi*0.5d0*lambda*mat3
