...
contains
  ...
  function h0_rk_ti(kx,ky,N) result(H)
    ...
    H = (mh-e0*cos(kx)-e0*cos(ky))*mat1 + lambda*(sin(ky)*mat2 - sin(kx)*mat3) + xi*mag(1)*matMagX + xi*mag(2)*matMagy + xi*mag(3)*matMagZ + 0.1*symBrk
  end function h0_rk_ti

  function t0_rk_ti(N) result(H)
    integer                    :: N
    complex(8),dimension(N,N)  :: H
    H = -0.5d0*e0*mat1 + xi*0.5d0*lambda*mat4
