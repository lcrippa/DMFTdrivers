program ed_ti_slab
  ...
  emat = kron( pauli_sigma_0, pauli_tau_z)
  !
  soxmat=kron( pauli_sigma_z, pauli_tau_x)
  soymat=kron( pauli_sigma_0, pauli_tau_y)
  sozmat=kron( pauli_sigma_x, pauli_tau_x)
  magmat=kron( pauli_sigma_z, pauli_tau_0)
  !
  GapOpeningMat=kron( pauli_sigma_y, pauli_tau_X)
  NHmat =zero
...
contains
  ...
  subroutine surface_gap(num)
    ...
    real(8)                                   :: coeff(N,Nlat*Nso),coeffreduced(N,Nlat),kpath(N,3),kx(N),siteindex(Nlat)
    ...
    do iband=BAND_CHECK-1,BAND_CHECK+2
      ...
      do ik=1,N
        ...
        coeff(ik,:)=Evecband*conjg(Evecband)
        do io=1,Nlat
          do iso=1,Nso
