program ed_ti_slab
  ...
  !
  call read_sigma_matsubara(Smats,20)
  do ilat=1,Nlat
...
contains
  ...
  subroutine read_sigma_matsubara(Selfenergy,Numberofsites)
    complex(8),dimension(Nlat,Nspin,Nspin,Norb,Norb,Lmats) :: Selfenergy
    character(len=30)                                      :: suffix
    integer                                                :: ilat,ispin,iorb,jorb,Numberofsites,fakeindex
      ...
      if(Nlat>4)then
        do ilat=3,Nlat-2
          do ispin=1,Nspin
            do iorb=1,Norb
              do jorb=1,Norb
                suffix="_l"//reg(txtfy(iorb))//reg(txtfy(jorb))//"_s"//reg(txtfy(ispin))//"_iw_ineq"//reg(txtfy(3,Npad=4))//".ed"
    ...
    !SECOND AND PENULTIMATE SITE
     do ispin=1,Nspin
        do iorb=1,Norb
          do jorb=1,Norb
            suffix="_l"//reg(txtfy(iorb))//reg(txtfy(jorb))//"_s"//reg(txtfy(ispin))//"_iw_ineq"//reg(txtfy(2,Npad=4))//".ed"
            call sread("impSigma"//trim(suffix),wm,Selfenergy(2,ispin,ispin,iorb,jorb,:))
            call sread("impSigma"//trim(suffix),wm,Selfenergy(Nlat-1,ispin,ispin,iorb,jorb,:))
          enddo
        enddo
     enddo
    !FIRST SITE
     do ispin=1,Nspin
        do iorb=1,Norb
          do jorb=1,Norb
            suffix="_l"//reg(txtfy(iorb))//reg(txtfy(jorb))//"_s"//reg(txtfy(ispin))//"_iw_ineq"//reg(txtfy(1,Npad=4))//".ed"
            call sread("impSigma"//trim(suffix),wm,Selfenergy(1,ispin,ispin,iorb,jorb,:))
          enddo
        enddo
     enddo
    !LAST SITE
     do ispin=1,Nspin
        do iorb=1,Norb
          do jorb=1,Norb
            suffix="_l"//reg(txtfy(iorb))//reg(txtfy(jorb))//"_s"//reg(txtfy(ispin))//"_iw_ineq"//reg(txtfy(Numberofsites,Npad=4))//".ed"
            call sread("impSigma"//trim(suffix),wm,Selfenergy(Nlat,ispin,ispin,iorb,jorb,:))
          enddo
        enddo
     enddo
    endif
  ...
  function find_x_coordinate(Nintervals) result(xcoord)
    ...
    do interval = 0,Nintervals-1
      xmin = -pi + pi/Nintervals*interval
      xmax = -pi + pi/Nintervals*(interval+1)
