program ed_ti_slab
  ...
  real(8)                                       :: GapOpeningField,NHfield_up,NHfield_dw,DUMMY,offset_x,offset_y,x_coord
  ...
  call parse_input_variable(x_coord,"X_COORD",finput,default=-3.d0)
  ...
  call get_state_localization(1000)
  !print*,"surface gap"
...
contains
  ...
  subroutine surface_gap(num)
    ...
    !
    !
    !kx = linspace(0.1d0,0.2d0,num)
    !ky = linspace(2.7d0,3.5d0,num)
    kx = linspace(-pi,pi,num)
    ky = linspace(-pi,pi,num)
    do ix=1,num
      do iy=1,num
    ...
    subroutine get_state_localization(N)
    integer                                   :: ipts,ik,ic,unit,iorb,ilat,io,nrot,u1,u2,N,iband,iso
    real(8)                                   :: coeff(N,Nlat*Nso),coeffreduced(N,Nlat),kpath(N,3),kx(N),siteindex(Nlat*Nso)
    ...
    !
    !
    kx = linspace(-1d0,1d0,N)
    do ik=1,N
      kpath(ik,:)=[kx(ik),0d0,0d0]*pi
    enddo
    do io=1,Nlat
      siteindex(io)=io
    enddo
    !
    do iband=BAND_CHECK-1,BAND_CHECK+2
      coeffreduced=zero
      do ik=1,N
        h = ti_edge_model(kpath(ik,:),Nlat,Nso,pbc) - XMU*Eye(Nlat*Nso)
        call eig(h,Eval,Evec)
        call sort_eigensystem(Eval,Evec)
        coeff(ik,:)=Evec(:,iband)*conjg(Evec(:,iband))
        coeff(ik,:)=coeff(ik,:)/sqrt(dot_product(Evec(:,iband),conjg(Evec(:,iband))))
        do io=1,Nlat
          do iso=1,Nso
            coeffreduced(ik,io) = coeffreduced(ik,io) + coeff(ik,(io-1)*Nso+iso)
          enddo
        enddo
      enddo
      call splot3d("state"//reg(txtfy(iband))//"_localization.dat",kx,siteindex,coeffreduced) 
    enddo
    !
