program ed_ti_slab
  ...
  if(Nspin/=2.OR.Norb/=2)stop "Wrong setup from input file: Nspin=2, Norb=2 -> 6Spin-Orbitals"
  Nso  = Nspin*Norb
  ...
  allocate(toconverge(Nineq,4,Lmats));toconverge=zero
  ...
  do while(.not.converged.AND.iloop<nloop)
     ...
     do ilat=1,Nineq
       toconverge(ilat,1,:) = Weiss_ineq(ilat,1,1,1,1,:)
       toconverge(ilat,2,:) = Weiss_ineq(ilat,2,2,1,1,:)
       toconverge(ilat,3,:) = Weiss_ineq(ilat,1,1,2,2,:)
       toconverge(ilat,4,:) = Weiss_ineq(ilat,2,2,2,2,:)
     enddo
     !
...
contains
  ...
  function h0_rk_ti(kx,kz,N) result(H)
    ...
    !
    H=Hblock
    H(1,1)=H(1,1)+0.5*e0*(cos(2*kx) + cos(2*kz))
    H(3,3)=H(3,3)+0.5*e0*(cos(2*kx) + cos(2*kz))
