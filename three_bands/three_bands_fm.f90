program ed_ti_slab
  ...
  do while(.not.converged.AND.iloop<nloop)
     ...
     !
     if(abs(dummymag) .gt. 1d-6) then
       S0=zero
       !dummymag=(1.1d0/sqrt(real(rank+1)))*dummymag/dummymag_rescale
       dummymag=2d0*dummymag/dummymag_rescale
       print*,"Dummymag for site ",rank," is ",dummymag
       spin_field_z=[0d0,dummymag,dummymag,0d0,0d0]
     endif
     !
     ...
     if(iloop > 10 .and. Refine_xmu) then
  ...
  subroutine read_sigma_real(Self)
    ...
    do ilat=1,Nineq
       do ispin=1,Nspin
          do iorb=1,Norb
            do jorb=1,Norb
             suffix="_l"//reg(txtfy(iorb))//reg(txtfy(jorb))//"_s"//reg(txtfy(ispin))//"_realw_ineq"//reg(txtfy(ilat,Npad=4))//".ed"
             call sread("impSigma"//trim(suffix),wr,Self_ineq(ilat,ispin,ispin,iorb,jorb,:))
            enddo
          enddo
       enddo
    enddo
  ...
  subroutine tune_xmu(xmu_converged)
    ...
    if(master)then
      ...
      if(abs(ep_energy-XMU)<0.01d0)then
        ...
        xmu_converged=.false.
        XMU=wmixing*XMU_REAL+(1d0-wmixing)*XMU_OLD
      endif
      XMU_OLD=XMU
    endif
