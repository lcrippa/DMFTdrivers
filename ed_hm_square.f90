program ed_hm_square
  ...
  USE MPI
  implicit none
  integer                                       :: iloop,Nb,Lk,Nx,Nso,ik,iorb,irepl,ispin
  logical                                       :: converged
  ...
  do while(.not.converged.AND.iloop<nloop)
     ...
     call dmft_write_gf(Gmats,"Gloc",axis='mats',iprint=3)
     ...
     call dmft_write_gf(Weiss,"Weiss",axis='mats',iprint=3)
     ...
     do ispin=1,Nspin
       do iorb=1,Norb
          Gtest=Gtest+Weiss(ispin,ispin,iorb,iorb,:)/Norb
       enddo
     enddo
